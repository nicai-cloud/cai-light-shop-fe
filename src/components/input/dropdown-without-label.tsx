...
export default function DropdownWithoutLabel(props: DropdownProps) {
    ...
    return (
        <Field className={tailwindMerge('relative', className)}>
            <Listbox as="div" value={value} onChange={handleChange} disabled={disabled}>
                ...
                >
                    <div className="text-p-small">{value.name}</div>
                    <Down
                ...
                <ListboxOptions
                    ...
                    className={tailwindMerge(
                        ...
                        'flex flex-col',
                        'bg-white rounded-sm border-2 border-black shadow-dialogue !overflow-hidden',
                        'focus:outline-none',
                        'transition',
                ...
                >
                    ...
                    >
                        {options.map(v => (
                            ...
                            >
                                <div className={tailwindMerge(
                                        'w-full text-left rounded-sm py-[2px] text-p text-dark-strong cursor-default select-none',
                                        'transition-colors duration-75 ease-in-out',
                                        'group-data-[focus]:bg-blue-100',
                                        'group-data-[selected]:text-black group-data-[selected]:font-bold'
                                    )}
                                >
                                    {v.name}
                                </div>
