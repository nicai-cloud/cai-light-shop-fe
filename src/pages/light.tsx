                ...
                <div className="px-2 mt-4">
                    ...
                    <div className="flex flex-row mt-4">
                        {Object.entries(colorsMapping!).map(([colorId, color]) => (
                            <div key={colorId} className={`w-[200px] border-2 ${selectedColorId! === Number(colorId) ? 'border-pink-300' : 'border-gray-100'} text-black text-center px-2 py-2 rounded mr-8`} onClick={() => handleSelectColor(Number(colorId))}>
                                {color.color}
                            </div>
                        ))}
                    </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-row mt-4">
                                {Object.entries(dimensionsMapping!).map(([dimensionId, dimension]) => (
                                    <div key={dimensionId} className={`w-[200px] border-2 ${selectedDimensionId! === Number(dimensionId) ? 'border-pink-300' : 'border-gray-100'} text-black text-center px-2 py-2 rounded mr-8`} onClick={() => handleSelectDimension(Number(dimensionId))}>
                                        {dimension.length}m
                                    </div>
                                ))}
                            </div>
